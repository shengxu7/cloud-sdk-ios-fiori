...
public struct ProfileHeader<Title: View, Subtitle: View, Footnote: View, DescriptionText: View, DetailImage: View>: View {
    ...
    public var body: some View {
        let configuration = ProfileHeaderStyleConfiguration(
            title: _title().modifier(Fiori.ProfileHeader.title.concat(titleModifier)).typeErased,
			subtitle: _subtitle().modifier(Fiori.ProfileHeader.subtitle.concat(subtitleModifier)).typeErased,
			footnote: _footnote().modifier(Fiori.ProfileHeader.footnote.concat(footnoteModifier)).typeErased,
			descriptionText: _descriptionText().modifier(Fiori.ProfileHeader.descriptionText.concat(descriptionTextModifier)).typeErased,
			detailImage: _detailImage().modifier(Fiori.ProfileHeader.detailImage.concat(detailImageModifier)).typeErased
        )
...
extension EnvironmentValues {
    var profileHeaderStyle: AnyProfileHeaderStyle {
        get { return self[ProfileHeaderStyleKey.self]  }
        set { self[ProfileHeaderStyleKey.self] = newValue }
    }
}
