...
public struct HeaderChart<Title: View, Subtitle: View, Trend: View, Kpi: View>: View {
    ...
    public var body: some View {
        let configuration = HeaderChartStyleConfiguration(
            title: _title().modifier(Fiori.HeaderChart.title.concat(titleModifier)).typeErased,
			subtitle: _subtitle().modifier(Fiori.HeaderChart.subtitle.concat(subtitleModifier)).typeErased,
			trend: _trend().modifier(Fiori.HeaderChart.trend.concat(trendModifier)).typeErased,
			kpi: _kpi().modifier(Fiori.HeaderChart.kpi.concat(kpiModifier)).typeErased
        )
...
extension EnvironmentValues {
    var headerChartStyle: AnyHeaderChartStyle {
        get { return self[HeaderChartStyleKey.self]  }
        set { self[HeaderChartStyleKey.self] = newValue }
    }
}
