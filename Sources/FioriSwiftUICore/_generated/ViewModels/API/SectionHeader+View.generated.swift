...
public struct SectionHeader<Title: View, Attribute: View>: View {
    ...
    public var body: some View {
        let configuration = SectionHeaderStyleConfiguration(
            title: _title().modifier(Fiori.SectionHeader.title.concat(titleModifier)).typeErased,
			attribute: _attribute().modifier(Fiori.SectionHeader.attribute.concat(attributeModifier)).typeErased
        )
...
extension EnvironmentValues {
    var sectionHeaderStyle: AnySectionHeaderStyle {
        get { return self[SectionHeaderStyleKey.self]  }
        set { self[SectionHeaderStyleKey.self] = newValue }
    }
}
